...
const int MAXMEMSIZEX = 2 * maxKernelSizeX + 1; //+16;
const int MAXMEMSIZEY = 2 * maxKernelSizeY + 1;// + 16;
...
__global__ void filter(unsigned char *image, unsigned char *out, const unsigned int imagesizex, const unsigned int imagesizey, const int kernelsizex, const int kernelsizey)
{
  ...
  int memBlockEndX = min(imagesizex-1, memBlockStartX + (int)(blockDim.x + 2*kernelsizex));
  int memBlockEndY = min(imagesizey-1, memBlockStartY + (int)(blockDim.y + 2*kernelsizey));
  ...
  for(int i = 0; i <= threadMem; i++) {
    // TODO: Find corresponding image data to memory index, no RGB?
  ...
  // Filter kernel (simple box filter)
	...
	for(dy=-kernelsizey;dy<=kernelsizey;dy++)
      ...
      // TODO: Replace Global Memory coordinates with Shared Memory coordinates sx and sy
      int xx = min(max(sx+dx, 0), memBlockEndX);
      int yy = min(max(sy+dy, 0), memBlockEndY);

      int sharedIndex = 3* (xx + memBlockSize*yy);

      // TODO: Instead, collect data from Shared Memory rather than Global Memory
