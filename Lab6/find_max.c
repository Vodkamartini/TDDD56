...
void runKernel(cl_kernel kernel, int threads, cl_mem data, unsigned int length, cl_mem localData)
{
  ...
  ciErrNum |= clSetKernelArg(kernel, 2, localWorkSize  * sizeof(unsigned int), NULL); // The error was: cl_enqueue_readbuffer cl_out_of_resources: Needed more local memory allocation; localWorkSize
...
int find_max_gpu(unsigned int *data, unsigned int length)
{
  ...
  localData = clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE, length * sizeof(unsigned int), NULL, &ciErrNum);

	printCLError(ciErrNum,7);

	// ********** RUN THE KERNEL ************
	runKernel(gpgpuReduction, length, io_data, length, localData);
...
// CPU max figet_group_idnder (sequential)
